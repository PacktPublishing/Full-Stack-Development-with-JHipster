    ...
    return (
      <div>
        ...
        <Table className="table table-striped table-bordered table-responsive d-table">
          ...
          <tbody>
            {this.sortPrefix(Object.keys(configProps), reversePrefix)
              .filter(this.filterFn)
              .map((configPropKey, configPropIndex) => (
                <tr key={configPropIndex}>
                  <td>
                    <span>{configProps[configPropKey].prefix}</span>
                  </td>
                  <td>
                    {this.sortProperties(Object.keys(configProps[configPropKey].properties), reverseProperties).map(
                      (propKey, propIndex) => (
        ...
        {Object.keys(env).map((envKey, envIndex) => (
          <div key={envIndex}>
            ...
            <Table className="table table-sm table-striped table-bordered table-responsive d-table">
              ...
              <tbody>
                {env[envKey]
                  ? Object.keys(env[envKey])
                      .filter(this.filterFn)
                      .map((propKey, propIndex) => (
                        <tr key={propIndex}>
                          <td className="break">{propKey}</td>
                          <td className="break">
                            <span className="float-right badge badge-secondary break">{JSON.stringify(env[envKey][propKey])}</span>
                          </td>
                        </tr>
                      ))
                  : ''}
